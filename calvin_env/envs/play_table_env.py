...
def get_env(dataset_path, obs_space=None, show_gui=True, **kwargs):
    ...
    ood_obj_orange_purple_gray = kwargs.get("ood_obj_orange_purple_gray", False)
    ood_obj_green = kwargs.get("ood_obj_green", False)
    ood_obj_yellow = kwargs.get("ood_obj_yellow", False)
    ood_obj_brown = kwargs.get("ood_obj_brown", False)

    # Only one red-only ood_obj color mode at a time (green/yellow/brown are mutually exclusive).
    _ood_obj_red_only_color = None
    if ood_obj_green:
        _ood_obj_red_only_color = ("green", 5)
    elif ood_obj_yellow:
        _ood_obj_red_only_color = ("yellow", 6)
    elif ood_obj_brown:
        _ood_obj_red_only_color = ("brown", 7)

    # ood_obj_orange_purple_gray and red-only modes are mutually exclusive.
    if ood_obj_orange_purple_gray and _ood_obj_red_only_color is not None:
        raise ValueError(
            "ood_obj_orange_purple_gray cannot be combined with "
            "ood_obj_green / ood_obj_yellow / ood_obj_brown."
        )

    # 1) Table (env) selection: ood_env can be combined with any ood_obj_* mode.
    if ood_env and ood_obj_orange_purple_gray:
        render_conf.scene.objects.fixed_objects.table.file = (
            'calvin_table_D/urdf/calvin_table_D_ood_env_obj.urdf'
        )
        print('OOD env + OOD obj (orange/purple/gray) table')
    elif ood_env:
        render_conf.scene.objects.fixed_objects.table.file = (
            'calvin_table_D/urdf/calvin_table_D_ood_env.urdf'
        )
        ...
        print('OOD env table')
    elif ood_obj_orange_purple_gray:
        render_conf.scene.objects.fixed_objects.table.file = (
            'calvin_table_D/urdf/calvin_table_D_ood_obj.urdf'
        )
        print('OOD obj (orange/purple/gray) table')

    # 2) Block (object) selection: independent of env, can be combined with ood_env.
    if ood_obj_orange_purple_gray:
        # OOD object colors (recolored block URDFs generated by calvin_color_customizer.py)
        #   block_red  -> orange (index 8) : blocks/block_red_middle_8.urdf
        #   block_blue -> purple (index 9) : blocks/block_blue_small_9.urdf
        #   block_pink -> gray   (index 10): blocks/block_pink_big_10.urdf
        render_conf.scene.objects.movable_objects.block_red.file = 'blocks/block_red_middle_8.urdf'
        render_conf.scene.objects.movable_objects.block_blue.file = 'blocks/block_blue_small_9.urdf'
        render_conf.scene.objects.movable_objects.block_pink.file = 'blocks/block_pink_big_10.urdf'
        print('OOD obj (orange/purple/gray) blocks')
    elif _ood_obj_red_only_color is not None:
        # OOD object colors: only block_red is recolored, blue/pink stay as default.
        #   block_red -> green  (index 5) : blocks/block_red_middle_5.urdf
        #   block_red -> yellow (index 6) : blocks/block_red_middle_6.urdf
        #   block_red -> brown  (index 7) : blocks/block_red_middle_7.urdf
        _color_name, _color_idx = _ood_obj_red_only_color
        render_conf.scene.objects.movable_objects.block_red.file = (
            f'blocks/block_red_middle_{_color_idx}.urdf'
        )
        print(f'OOD obj (red -> {_color_name}) blocks')
    #virtualkss end
